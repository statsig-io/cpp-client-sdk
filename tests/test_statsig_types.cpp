...
TEST(EvaluatedSpecTest, ConstructorWithJsonValue) {
  ...
  auto value = ValueMap({{"key1", "value1"}, {"key2", 2}});
  ...
  EXPECT_EQ(dynamic_config.GetValues().GetStringValue("key1"), "value1");
  EXPECT_EQ(dynamic_config.GetValues().GetValue("key2"), 2);
  EXPECT_EQ(dynamic_config.GetValues().GetValue("key3"), nullptr);
}
...
TEST(EvaluatedSpecTest, ConstructorWithJsonObjValue) {
  ...
  EXPECT_EQ(experiment.GetValues().GetStringValue("key1"), "value1");
  EXPECT_EQ(experiment.GetValues().GetValue("key2"), 2);
  EXPECT_EQ(experiment.GetValues().GetValue("key3"), nullptr);
}
...
TEST(EvaluatedSpecTest, GetValueFromLayer) {
  ...
  EXPECT_EQ(retrieved_value, value.GetValue(parameter_name));
