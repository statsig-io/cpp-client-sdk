...
TEST(StatsigClientThreadedTest, DoesNotBlock) {
  ...
  mock_adapter->on_get_data_async = [](auto, auto, auto) { sleep(5); };

  StatsigOptions opts;
  opts.data_adapter = mock_adapter;

  auto completed = RunBlocking(1000, [&client, &opts](auto done) {
    client.InitializeAsync("client-test", [](auto) {}, std::nullopt, opts);
    done();
  });

  EXPECT_TRUE(completed) << "Asynchronous task did not complete within the timeout";
}